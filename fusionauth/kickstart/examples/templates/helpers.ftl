...
[#macro head title="Login | Company Name" author="Company Name" description="Company Name user management."]
<head>
  <title>${title}</title>
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  <meta name="application-name" content="FusionAuth Company Name">
  ...
  <script>
    ...
    Prime.Document.onReady(function() {
      Prime.Document.query('.notification').each(function(e) {
        var dismissButton = e.queryFirst('button.delete');
...
<body class="has-background-light">
  ...
  <script src="https://unpkg.com/buefy/dist/components/navbar"></script>
  <script src="https://unpkg.com/buefy/dist/components/progress"></script>
  <script>
    const logoutDuration = 3
    const duration = logoutDuration * 1000
    const step = logoutDuration * 10
      ...
      window.Progress = new Vue({
        ...
        mounted() {
          const updateProgress = setInterval(this.incrementProgress, step);
          if (this.progress === 100) clearInterval(updateProgress)
        },
...
[#macro header]
  <header id="header">
      <b-navbar type="is-black" spaced shadow>
        <template slot="brand">
          <b-navbar-item tag="a" href="/">
            <img class="image"
              src=""
              alt="Company Name Logo">
            <span class="ml-2 is-size-5 is-uppercase has-text-weight-semibold is-hidden-mobile">Company Name</span>
          </b-navbar-item>
        </template>


        <template slot="end">
          <b-navbar-item tag="a" target="_blank" href="https://fusionauth.io/docs">
            <span class="icon"><i class="fas fa-question-circle mr-2"></i></span>
            ${theme.message('help')}
          </b-navbar-item>
        </template>
      </b-navbar>
...
<main>
  ...
  <section class="main-content">
      <div class="card">
        ...
        <h4 class="is-size-4 mb-4">${title}</h4>
        [/#if]
          [#nested/]
        </div>
      </div>
...
[#macro footer]
  ...
  <footer class="footer has-background-black">
    <div class="content has-text-centered">
      <p>Powered by <a href="https://fusionauth.io" target="_blank">FusionAuth</a> </p>
    </div>
  </footer>
[/#macro]
...
[#macro printErrorAlerts]
  [#if errorMessages?size > 0]
    [#list errorMessages as m]
      [@alert message=m type="is-danger" icon="exclamation-circle"/]
    [/#list]
  [/#if]
[/#macro]
...
[#macro printInfoAlerts]
  [#if infoMessages?size > 0]
    [#list infoMessages as m]
      [@alert message=m type="is-info" icon="info-circle"/]
    [/#list]
  [/#if]
[/#macro]

[#macro alert message type icon includeDismissButton=true]
<div class="my-2 notification is-light ${type}">
...
<div class="field">
  [#if label?has_content][#t/]
    <label for="${id}"[#if (fieldMessages[name]![])?size > 0] class="has-text-danger"[/#if]>${label}[#if required] <span class="has-text-danger">*</span>[/#if][#t/]
    [#if tooltip?has_content][#t/]
    <span class="icon has-tooltip-right has-tooltip-arrow" data-tooltip="${tooltip}">[#t/]
      <i class="fas fa-info-circle"></i>[#t/]
    <span>[#t/]
    [/#if][#t/]
    </label>[#t/]
  [/#if]
  [#if leftAddon?has_content]
    <div class="control has-icons-left">
      <span class="icon is-small is-left">
        <i class="fas fa-${leftAddon}"></i>
      </span>
  [#else]
    <div class="control">
  [/#if]
    <input type="${type}" name="${name}" [#if type != "password"]value="${(name?eval!'')}"[/#if] class="input ${class}" autocapitalize="${autocapitalize}" autocomplete="${autocomplete}" autocorrect="${autocorrect}" spellcheck="${spellcheck}" [#if autofocus]autofocus="autofocus"[/#if] placeholder="${placeholder}[#if required] *[/#if]" [#if disabled]disabled="disabled"[/#if]/>
  [#if dateTimeFormat != ""]
    <input type="hidden" name="${name}@dateTimeFormat" value="${dateTimeFormat}"/>
  [/#if]
    </div>
...
<p class="help is-danger">[#list fieldMessages[field] as message]${message?no_esc}[#if message_has_next], [/#if][/#list]</p>
[/#if]
[/#macro]

[#macro button text icon="arrow-right" color="" disabled=false]
<button class="${color} button is-rounded"[#if disabled] disabled[/#if]>
